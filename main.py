...
class MainWindow(QMainWindow):
    ...
    def undo(self):
        ...
        if not self.history:
            return
        # History'den önceki durumu al, ama sağ kanvasını boş bırak
        prev_current, prev_output = self.history.pop()
        self.current_image = prev_current
        self.output_image = None
        self._refresh_view()
    ...
    def accept_output(self):
        ...
        # Önceki current_image ve output_image'ı birlikte kaydet
        if self.current_image is not None:
            self.history.append((self.current_image.copy(), self.output_image.copy() if self.output_image is not None else None))
